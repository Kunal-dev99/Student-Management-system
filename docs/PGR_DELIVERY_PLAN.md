...
| 4A Make it real | **6/6** | **5/5** | **COMPLETE** — worker (APScheduler) + outbox retry/dead-letter, file storage, email+prefs, auth hardening (revocation/lockout/reset), audit trail. Verified live in-browser. |
| 4B Module depth | **4/4** | **4/4** | Thesis (examiner CoI, viva scheduling, corrections), Supervision (meeting log, compliance, capacity), Progression (panels, conditions, appeals), Funding (cost centres, stipend schedule, fee waivers). Migrations through `3a5a0cd9be93`. Verified live in-browser. See `PGR_PHASE4_PLAN.md`. |
| 6 Vision gap closure | 6.0 ✅ · 6.5 ✅ · 6.1 ✅ | UI in progress | Closing the CIO gap analysis. 6.0 proved routes + employee continuity (GAP-02/04 closed); 6.5 suspension/extension lifecycle with approval + deterministic recalculation (GAP-06); 6.1 research award/demand + position provenance + lineage (GAP-01). Remaining: 6.2 routes UI, 6.3 funding lineage, 6.4 employee endpoints, 6.6 statutory HESA. See `PGR_PHASE6_GAP_CLOSURE_PLAN.md`. |
| 5.1 Ask PGR assistant | **done** | **done** | **COMPLETE — rules-first, no API key needed.** On-premise intent parser (`intents.py`: synonyms, negation, duration extraction, proximity binding) drives `cohort_query` + lookups + navigation at zero cost with no data leaving the server; LLM fallback is opt-in and OFF by default. Floating "Ask PGR" launcher + Cmd+K palette, answers carry an `understood` readback and per-student match reasons; unmatched questions decline with suggestions. `assistant.use` gate (admins only). **114/114 tests**, build clean, verified in-browser. See `PGR_ASSISTANT_DESIGN.md`. |
